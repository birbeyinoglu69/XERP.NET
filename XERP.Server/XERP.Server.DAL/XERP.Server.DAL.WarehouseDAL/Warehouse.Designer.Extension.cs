...
namespace ExtensionMethods
{
    ...
    {
        #region Warehouse Extension Methods
        public static List<Temp> GetMetaData(this Warehouse entityObject)
        {
        ...
        #endregion Warehouse Extension Methods

        #region WarehouseLocationBin Extension Methods
        ...
        public static List<Temp> GetMetaData(this WarehouseLocationBinCode entityObject)
        {
            XERP.Server.DAL.WarehouseDAL.DALUtility dalUtility = new DALUtility();
            List<Temp> tempList = new List<Temp>();
            int id = 0;
            using (WarehouseEntities ctx = new WarehouseEntities(dalUtility.EntityConectionString))
            {
                ...
                var c = ctx.WarehouseLocationBinCodes.FirstOrDefault();
                var queryResults = from meta in ctx.MetadataWorkspace.GetItems(DataSpace.CSpace)
                                                        .Where(m => m.BuiltInTypeKind == BuiltInTypeKind.EntityType)
                                   from query in (meta as EntityType).Properties
                                           .Where(p => p.DeclaringType.Name == entityObject.GetType().Name)
        ...
        #endregion WarehouseLocationBin Extension Methods
    }
}
