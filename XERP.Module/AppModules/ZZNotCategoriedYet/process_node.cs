
using System;
using System.ComponentModel;
using DevExpress.Xpo;
using DevExpress.ExpressApp;
using DevExpress.Persistent.Base;
using DevExpress.Persistent.BaseImpl;
using DevExpress.Persistent.Validation;
using DevExpress.Persistent.Base.General;
using DevExpress.Data.Filtering;
using System.Collections.Generic;

namespace XERP
{



    [DefaultClassOptions]
    [DeferredDeletion(true)]
	[DefaultProperty("kind")]
    [Persistent("process_node")]
	public partial class process_node : XPCustomObject
	{
		#region Properties
	    private System.Int32 fid;
        [Key(AutoGenerate = true), Browsable(false)]
        //process_node_id
		public System.Int32 id {
			get { return fid; }
			set { SetPropertyValue("id", ref fid, value); }
		}
        
            private res_users fcreate_uid;
            //FK FK_process_node_create_uid
            [Custom("Caption", "Create Uid")]
            public res_users create_uid {
                get { return fcreate_uid; }
                set { SetPropertyValue<res_users>("create_uid", ref fcreate_uid, value); }
            }
    
            private DateTime? fcreate_date;
            [Custom("Caption", "Create Date")]
            public DateTime? create_date {
                get { return fcreate_date; }
                set { SetPropertyValue("create_date", ref fcreate_date, value); }
            }
    
            private DateTime? fwrite_date;
            [Custom("Caption", "Write Date")]
            public DateTime? write_date {
                get { return fwrite_date; }
                set { SetPropertyValue("write_date", ref fwrite_date, value); }
            }
    
        
            private res_users fwrite_uid;
            //FK FK_process_node_write_uid
            [Custom("Caption", "Write Uid")]
            public res_users write_uid {
                get { return fwrite_uid; }
                set { SetPropertyValue<res_users>("write_uid", ref fwrite_uid, value); }
            }
    
        
            private ir_ui_menu fmenu_id;
            //FK FK_process_node_menu_id
            [Custom("Caption", "Menu Id")]
            public ir_ui_menu menu_id {
                get { return fmenu_id; }
                set { SetPropertyValue<ir_ui_menu>("menu_id", ref fmenu_id, value); }
            }
    
        
            private ir_model fmodel_id;
            //FK FK_process_node_model_id
            [Custom("Caption", "Model Id")]
            public ir_model model_id {
                get { return fmodel_id; }
                set { SetPropertyValue<ir_model>("model_id", ref fmodel_id, value); }
            }
    
            private System.String fkind;
            [Size(16)]
            [Custom("Caption", "Kind")]
            public System.String kind {
                get { return fkind; }
                set { SetPropertyValue("kind", ref fkind, value); }
            }
    
            private System.String fnote;
            [Size(-1)]
            [Custom("Caption", "Note")]
            public System.String note {
                get { return fnote; }
                set { SetPropertyValue("note", ref fnote, value); }
            }
    
            private System.String fname;
            [Size(30)]
            [Custom("Caption", "Name")]
            public System.String name {
                get { return fname; }
                set { SetPropertyValue("name", ref fname, value); }
            }
    
        
            private process_process fsubflow_id;
            //FK FK_process_node_subflow_id
            [Custom("Caption", "Subflow Id")]
            public process_process subflow_id {
                get { return fsubflow_id; }
                set { SetPropertyValue<process_process>("subflow_id", ref fsubflow_id, value); }
            }
    
        
            private process_process fprocess_id;
            //FK FK_process_node_process_id
            [Custom("Caption", "Process Id")]
            public process_process process_id {
                get { return fprocess_id; }
                set { SetPropertyValue<process_process>("process_id", ref fprocess_id, value); }
            }
    
            private System.String fmodel_state1s;
            [Size(128)]
            [Custom("Caption", "Model State1s")]
            public System.String model_state1s {
                get { return fmodel_state1s; }
                set { SetPropertyValue("model_state1s", ref fmodel_state1s, value); }
            }
    
            private System.String fhelp_url;
            [Size(255)]
            [Custom("Caption", "Help Url")]
            public System.String help_url {
                get { return fhelp_url; }
                set { SetPropertyValue("help_url", ref fhelp_url, value); }
            }
    
            private System.Boolean fflow_start;
            [Custom("Caption", "Flow Start")]
            public System.Boolean flow_start {
                get { return fflow_start; }
                set { SetPropertyValue("flow_start", ref fflow_start, value); }
            }
    
        
            private document_directory fdirectory_id;
            //FK FK_process_node_directory_id
            [Custom("Caption", "Directory Id")]
            public document_directory directory_id {
                get { return fdirectory_id; }
                set { SetPropertyValue<document_directory>("directory_id", ref fdirectory_id, value); }
            }
    
		#endregion
	
		#region Collections
		#endregion
	
		#region Constructors
		public process_node(Session session) : base(session) { }
        #endregion

	}
}
//Generated for XERP

