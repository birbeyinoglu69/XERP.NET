     ...
     #region Initialization and Cleanup
        ...
        private void OnSelectedKeyChanged()
        {
            ...
            if ((_companyTypeID != SelectedCompanyType.CompanyTypeID &&
                GetCompanyTypeState(_selectedCompanyType) != EntityStates.Added))
            {
                if (_dirty)
                {
        ...
        private bool Update(CompanyType obj)
        {
            //if it is added make sure it does not exist allready...
            if (GetCompanyTypeState(_selectedCompanyType) == EntityStates.Added)
            {
                if (CompanyTypeExists(SelectedCompanyType.CompanyTypeID.ToString()))
                {
                    Exception e = new Exception();
                    NotifyError("CompanyID " + _selectedCompanyType.CompanyTypeID + " Allready Exists", e);
                    return false;
                }
            }
            _serviceAgent.UpdateCompanyTypeRepository(obj);
            _dirty = true;
            return true;
        }
        ...
        {
            if (GetCompanyTypeState(_selectedCompanyType) != EntityStates.Detached)
            {
