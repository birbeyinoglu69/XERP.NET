...
{
    /// <summary>
    /// Interaction logic for WarehouseMaintenanceView.xaml
    /// </summary>
    public partial class MainMaintenanceView : UserControl
    {
        ...
        public MainMaintenanceView()
        {
            try
            {
                this.Resources.MergedDictionaries.Add(XERP.Client.WPF.WarehouseMaintenance.Resources.SharedDictionaryManager.MenuImagesSharedDictionary);
                this.Resources.MergedDictionaries.Add(XERP.Client.WPF.WarehouseMaintenance.Resources.SharedDictionaryManager.BaseControlsSharedDictionary);
        ...
        {
            XERP.Client.WPF.PlantMaintenance.MainWindow window = new XERP.Client.WPF.PlantMaintenance.MainWindow();
            window.Show();
        }

        private void OpenAddressMaintenance_Click(object sender, RoutedEventArgs e)
        {
            XERP.Client.WPF.AddressMaintenance.MainWindow window = new XERP.Client.WPF.AddressMaintenance.MainWindow();
            window.Show();
        }
        ...
        private void dgMain_PreviewKeyUp(object sender, KeyEventArgs e)
        {
            if (e.Key == Key.Enter)
            {
                ...
                {
                    _viewModel.NewWarehouseCommand("");
                ...
                {
                    MessageBox.Show("New Warehouse Is Not Enabled...", "Error", MessageBoxButton.OK, MessageBoxImage.Error);
                }
            }
        ...
        private void DataGrid_Initialized(object sender, EventArgs e)
        {
            ...
            foreach (DataGridColumn dataGridColumn in datagrid.Columns)
            {
                if (dataGridColumn is DataGridTextColumn)
                {
                    ...
                    int maxColumnLength;
                    if (!_viewModel.WarehouseMaxFieldValueDictionary.TryGetValue(binding.Path.Path.ToString(), out maxColumnLength))
                        continue;
        ...
        {
            _viewModel.SelectedWarehouseList = dgMain.SelectedItems;
        ...
        {//The columns may get moved so we need to predicate the column order before
            ...
            _viewModel.WarehouseColumnMetaDataList = new List<ColumnMetaData>();
            foreach (DataGridColumn column in dgMain.Columns)
            {
                ...
                _viewModel.WarehouseColumnMetaDataList.Add(columnMetaData);
            }        
        ...
        private void dgMain_PreviewKeyDown(object sender, KeyEventArgs e)
        {
            if (e.Key == Key.Delete)
            {
                ...
                {
                    _viewModel.DeleteWarehouseCommand();
                    return;
                }
