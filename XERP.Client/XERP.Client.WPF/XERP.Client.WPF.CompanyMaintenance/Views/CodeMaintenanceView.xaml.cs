...
{
    /// <summary>
    /// Interaction logic for CompanyCodeMaintenanceView.xaml
    /// </summary>
    public partial class CodeMaintenanceView : UserControl
    {
        ...
        private void DataGrid_Initialized(object sender, EventArgs e)
        {
            ...
            foreach (DataGridColumn dataGridColumn in datagrid.Columns)
            {
                if (dataGridColumn is DataGridTextColumn)
                {
                    ...
                    int maxColumnLength;
                    if (!_viewModel.CompanyCodeMaxFieldValueDictionary.TryGetValue(binding.Path.Path.ToString(), out maxColumnLength))
                        continue;
        ...
        {
            _viewModel.SelectedCompanyCodeList = dgMain.SelectedItems;
        ...
        {//The columns may get moved so we need to predicate the column order before
            ...
            _viewModel.CompanyCodeColumnMetaDataList = new List<ColumnMetaData>();
            foreach (DataGridColumn column in dgMain.Columns)
            {
                ...
                _viewModel.CompanyCodeColumnMetaDataList.Add(columnMetaData);
            }
        ...
        private void dgMain_PreviewKeyDown(object sender, KeyEventArgs e)
        {
            if (e.Key == Key.Delete)
            {
                ...
                {
                    _viewModel.DeleteCompanyCodeCommand();
                    return;
                }
