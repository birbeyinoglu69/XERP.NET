...
namespace XERP.Client.WPF.PlantMaintenance.ViewModels
{
    public class TypeSearchViewModel : ViewModelBase<TypeSearchViewModel>
    {
        ...
        private PlantType _searchObject;
        public PlantType SearchObject
        {
        ...
        private BindingList<PlantType> _resultList;
        public BindingList<PlantType> ResultList
        {
        ...
        {
            return new BindingList<PlantType>(_serviceAgent.GetPlantTypes(companyID).ToList());
        }

        private BindingList<PlantType> GetPlantTypes(PlantType itemQueryObject, string companyID)
        {
            return new BindingList<PlantType>(_serviceAgent.GetPlantTypes(itemQueryObject, companyID).ToList());
        }
        #endregion Methods
        ...
        {
            ResultList = GetPlantTypes(SearchObject, ClientSessionSingleton.Instance.CompanyID);
        }
        ...
        public void CommitSearchCommand()
        {
            ...
            {
                BindingList<PlantType> selectedList = new BindingList<PlantType>();
                foreach (var item in SelectedList)
                {
                    selectedList.Add((PlantType)item);
                }
                MessageBus.Default.Notify(MessageTokens.PlantTypeSearchToken.ToString(), this, new NotificationEventArgs<BindingList<PlantType>>("", selectedList));
            }
            NotifyClose("");
        }
        #endregion Commands
