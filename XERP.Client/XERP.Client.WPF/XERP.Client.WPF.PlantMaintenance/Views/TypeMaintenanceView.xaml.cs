...
{
    /// <summary>
    /// Interaction logic for PlantTypeMaintenanceView.xaml
    /// </summary>
    public partial class TypeMaintenanceView : UserControl
    {
        ...
        private void dgMain_PreviewKeyUp(object sender, KeyEventArgs e)
        {
            if (e.Key == Key.Enter)
            {
                ...
                {
                    _viewModel.NewPlantTypeCommand("");
                ...
                {
                    MessageBox.Show("New Plant Type Is Not Enabled...", "Error", MessageBoxButton.OK, MessageBoxImage.Error);
                }
            }
        ...
        private void DataGrid_Initialized(object sender, EventArgs e)
        {
            ...
            foreach (DataGridColumn dataGridColumn in datagrid.Columns)
            {
                if (dataGridColumn is DataGridTextColumn)
                {
                    ...
                    int maxColumnLength;
                    if (!_viewModel.PlantTypeMaxFieldValueDictionary.TryGetValue(binding.Path.Path.ToString(), out maxColumnLength))
                        continue;
        ...
        {
            _viewModel.SelectedPlantTypeList = dgMain.SelectedItems;
        ...
        {//The columns may get moved so we need to predicate the column order before
            ...
            _viewModel.PlantTypeColumnMetaDataList = new List<ColumnMetaData>();
            foreach (DataGridColumn column in dgMain.Columns)
            {
                ...
                _viewModel.PlantTypeColumnMetaDataList.Add(columnMetaData);
            }        
        ...
        private void dgMain_PreviewKeyDown(object sender, KeyEventArgs e)
        {
            if (e.Key == Key.Delete)
            {
                ...
                {
                    _viewModel.DeletePlantTypeCommand();
                    return;
                }
