...
{
    /// <summary>
    /// Interaction logic for WarehouseLocationCodeMaintenanceView.xaml
    /// </summary>
    public partial class CodeMaintenanceView : UserControl
    {
        ...
        private void dgMain_PreviewKeyUp(object sender, KeyEventArgs e)
        {
            if (e.Key == Key.Enter)
            {
                ...
                {
                    _viewModel.NewWarehouseLocationCodeCommand("");
                ...
                {
                    MessageBox.Show("New WarehouseLocation Code Is Not Enabled...", "Error", MessageBoxButton.OK, MessageBoxImage.Error);
                }
            }
        ...
        private void DataGrid_Initialized(object sender, EventArgs e)
        {
            ...
            foreach (DataGridColumn dataGridColumn in datagrid.Columns)
            {
                if (dataGridColumn is DataGridTextColumn)
                {
                    ...
                    int maxColumnLength;
                    if (!_viewModel.WarehouseLocationCodeMaxFieldValueDictionary.TryGetValue(binding.Path.Path.ToString(), out maxColumnLength))
                        continue;
        ...
        {
            _viewModel.SelectedWarehouseLocationCodeList = dgMain.SelectedItems;
        ...
        {//The columns may get moved so we need to predicate the column order before
            ...
            _viewModel.WarehouseLocationCodeColumnMetaDataList = new List<ColumnMetaData>();
            foreach (DataGridColumn column in dgMain.Columns)
            {
                ...
                _viewModel.WarehouseLocationCodeColumnMetaDataList.Add(columnMetaData);
            }
        ...
        private void dgMain_PreviewKeyDown(object sender, KeyEventArgs e)
        {
            if (e.Key == Key.Delete)
            {
                ...
                {
                    _viewModel.DeleteWarehouseLocationCodeCommand();
                    return;
                }
