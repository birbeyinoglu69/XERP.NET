...
using XERP.Client.WPF.WarehouseLocationMaintenance.ViewModels;
using SimpleMvvmToolkit;
using System.Collections.Generic;
using XERP.Client.Models;
using XERP.Client.WPF;
namespace XERP.Client.WPF.WarehouseLocationMaintenance.Views
{
    /// <summary>
    /// Interaction logic for WarehouseLocationMaintenanceView.xaml
    /// </summary>
    public partial class MainMaintenanceView : UserControl
    {
        ...
        public MainMaintenanceView()
        {
            try
            {
                this.Resources.MergedDictionaries.Add(XERP.Client.WPF.WarehouseLocationMaintenance.Resources.SharedDictionaryManager.MenuImagesSharedDictionary);
                this.Resources.MergedDictionaries.Add(XERP.Client.WPF.WarehouseLocationMaintenance.Resources.SharedDictionaryManager.BaseControlsSharedDictionary);
        ...
        private void dgMain_PreviewKeyUp(object sender, KeyEventArgs e)
        {
            if (e.Key == Key.Enter)
            {
                ...
                {
                    _viewModel.NewWarehouseLocationCommand("");
                ...
                {
                    MessageBox.Show("New WarehouseLocation Is Not Enabled...", "Error", MessageBoxButton.OK, MessageBoxImage.Error);
                }
            }
        ...
        private void DataGrid_Initialized(object sender, EventArgs e)
        {
            ...
            foreach (DataGridColumn dataGridColumn in datagrid.Columns)
            {
                if (dataGridColumn is DataGridTextColumn)
                {
                    ...
                    int maxColumnLength;
                    if (!_viewModel.WarehouseLocationMaxFieldValueDictionary.TryGetValue(binding.Path.Path.ToString(), out maxColumnLength))
                        continue;
        ...
        {
            _viewModel.SelectedWarehouseLocationList = dgMain.SelectedItems;
        ...
        {//The columns may get moved so we need to predicate the column order before
            ...
            _viewModel.WarehouseLocationColumnMetaDataList = new List<ColumnMetaData>();
            foreach (DataGridColumn column in dgMain.Columns)
            {
                ...
                _viewModel.WarehouseLocationColumnMetaDataList.Add(columnMetaData);
            }        
        ...
        private void dgMain_PreviewKeyDown(object sender, KeyEventArgs e)
        {
            if (e.Key == Key.Delete)
            {
                ...
                {
                    _viewModel.DeleteWarehouseLocationCommand();
                    return;
                }
