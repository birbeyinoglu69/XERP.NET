...
namespace XERP.Client.WPF.AddressMaintenance.Views
{
    /// <summary>
    /// Interaction logic for AddressMaintenanceView.xaml
    /// </summary>
    public partial class MainMaintenanceView : UserControl
    {
        ...
        private void dgMain_PreviewKeyUp(object sender, KeyEventArgs e)
        {
            if (e.Key == Key.Enter)
            {
                ...
                {
                    MessageBox.Show("New Address Is Not Enabled...", "Error", MessageBoxButton.OK, MessageBoxImage.Error);
                }
            }
        ...
        private void DataGrid_Initialized(object sender, EventArgs e)
        {
            ...
            foreach (DataGridColumn dataGridColumn in datagrid.Columns)
            {
                if (dataGridColumn is DataGridTextColumn)
                {
                    ...
                    int maxColumnLength;
                    if (!_viewModel.AddressMaxFieldValueDictionary.TryGetValue(binding.Path.Path.ToString(), out maxColumnLength))
                        continue;
        ...
        {
            _viewModel.SelectedAddressList = dgMain.SelectedItems;
        ...
        private void dgMain_PreviewKeyDown(object sender, KeyEventArgs e)
        {
            if (e.Key == Key.Delete)
            {
                ...
                {
                    _viewModel.DeleteAddressCommand();
                    return;
                }
