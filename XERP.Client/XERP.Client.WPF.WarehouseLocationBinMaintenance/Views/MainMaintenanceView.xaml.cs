...
using XERP.Client.WPF.WarehouseLocationBinMaintenance.ViewModels;
using SimpleMvvmToolkit;
using System.Collections.Generic;
using XERP.Client.Models;
using XERP.Client.WPF;
namespace XERP.Client.WPF.WarehouseLocationBinMaintenance.Views
{
    /// <summary>
    /// Interaction logic for WarehouseLocationBinMaintenanceView.xaml
    /// </summary>
    public partial class MainMaintenanceView : UserControl
    {
        ...
        public MainMaintenanceView()
        {
            try
            {
                this.Resources.MergedDictionaries.Add(XERP.Client.WPF.WarehouseLocationBinMaintenance.Resources.SharedDictionaryManager.MenuImagesSharedDictionary);
                this.Resources.MergedDictionaries.Add(XERP.Client.WPF.WarehouseLocationBinMaintenance.Resources.SharedDictionaryManager.BaseControlsSharedDictionary);
        ...
        private void OpenWarehouseMaintenance_Click(object sender, RoutedEventArgs e)
        {
            XERP.Client.WPF.WarehouseMaintenance.MainWindow maintenanceWindow = new XERP.Client.WPF.WarehouseMaintenance.MainWindow();
            maintenanceWindow.Show();
        }
        ...
        private void dgMain_PreviewKeyUp(object sender, KeyEventArgs e)
        {
            if (e.Key == Key.Enter)
            {
                ...
                {
                    _viewModel.NewWarehouseLocationBinCommand("");
                ...
                {
                    MessageBox.Show("New WarehouseLocationBin Is Not Enabled...", "Error", MessageBoxButton.OK, MessageBoxImage.Error);
                }
            }
        ...
        private void DataGrid_Initialized(object sender, EventArgs e)
        {
            ...
            foreach (DataGridColumn dataGridColumn in datagrid.Columns)
            {
                if (dataGridColumn is DataGridTextColumn)
                {
                    ...
                    int maxColumnLength;
                    if (!_viewModel.WarehouseLocationBinMaxFieldValueDictionary.TryGetValue(binding.Path.Path.ToString(), out maxColumnLength))
                        continue;
        ...
        {
            _viewModel.SelectedWarehouseLocationBinList = dgMain.SelectedItems;
        ...
        {//The columns may get moved so we need to predicate the column order before
            ...
            _viewModel.WarehouseLocationBinColumnMetaDataList = new List<ColumnMetaData>();
            foreach (DataGridColumn column in dgMain.Columns)
            {
                ...
                _viewModel.WarehouseLocationBinColumnMetaDataList.Add(columnMetaData);
            }        
        ...
        private void dgMain_PreviewKeyDown(object sender, KeyEventArgs e)
        {
            if (e.Key == Key.Delete)
            {
                ...
                {
                    _viewModel.DeleteWarehouseLocationBinCommand();
                    return;
                }
